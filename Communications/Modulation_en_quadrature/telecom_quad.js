/*
Modulateur - démodulateur en quadrature v1.0
		...
		switch(forme) {
			...
			case 2 :
				for (let index=0;index<this.N;index++) {
					...
					let s=offset;
					for (let h=0;h<15;h++)
						s=s+amp/(2*h+1)*Math.sin(2*Math.PI*index*this.Tech*freq*(2*h+1)+phase/180*Math.PI)*4/Math.PI;
					this.signal.push(s);
				}
				break;
			...
			case 3 :
				for (let index=0;index<this.N;index++) {
					...
					let s=offset;
					for (let h=0;h<10;h++)
						s=s+amp/(2*h+1)/(2*h+1)*((-1)**h)*Math.sin(2*Math.PI*index*this.Tech*freq*(2*h+1)+phase/180*Math.PI)*8/Math.PI/Math.PI;
					this.signal.push(s);
				}
				break;
			...
			case 4 :
				...
				for (let index=0;index<this.N;index++) {
					if ((2*Math.PI*index*this.Tech*freq+phase/180*Math.PI) % 2*Math.PI < 2*Math.PI*this.Tech*freq+phase/180*Math.PI)
...
const valBaseTps=[0.1e-3,1e-3,10e-3,100e-3,1.];
